...
if [ -n "$COMPARE_RAW" ]; then
  ...
  node trace-tools/compare-traces.js "${TRACE_FILES[0]}" "${TRACE_FILES[1]}"
elif [ -n "$COMPARE_SEMANTIC" ]; then
  if [ ${#TRACE_FILES[@]} -lt 2 ]; then
    echo "Error: --compare-semantic requires two trace files"
    exit 1
  fi
  node trace-tools/compare-traces.js --semantic $SHOW_JOURNEY "${TRACE_FILES[0]}" "${TRACE_FILES[1]}"
elif [ -n "$RUN" ]; then
  # Generate test in trace-tools (matches playwright.config.ts testDir)
  TEST_FILE="trace-tools/generated-${TEST_NAME}.spec.ts"
  BASELINE="${TRACE_FILES[0]}"
  node trace-tools/generate-playwright.js "$BASELINE" "$TEST_NAME" > "$TEST_FILE"
  ...
  if [ -f "captured-trace.json" ]; then
    ...
    node trace-tools/compare-traces.js --semantic "$BASELINE" captured-trace.json
  else
  ...
  node trace-tools/generate-playwright.js "${TRACE_FILES[0]}" "$TEST_NAME"
else
  node trace-tools/summarize.js $SHOW_JOURNEY "${TRACE_FILES[0]}"
fi
